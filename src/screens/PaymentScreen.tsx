...
const PaymentScreen = () => {
  ...
  const payMethods = [
    { id: 'mtn', name: 'MTN Mobile Money', sub: 'Recommended', emoji: '💛', bgClass: 'bg-gradient-to-br from-yellow-400 to-amber-500', badge: true },
    { id: 'airtel', name: 'Airtel Money', sub: '0739 XXX XXX', emoji: '❤️', bgClass: 'bg-gradient-to-br from-red-500 to-red-700' },
    { id: 'card', name: 'Bank Card', sub: 'Coming soon', emoji: '💳', bgClass: 'bg-muted', disabled: true },
  ];
    ...
    <div className="flex flex-col min-h-screen bg-background">
      <div className="flex-1 pb-4">
        ...
        <div className="px-5 pt-14 pb-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[17px] font-extrabold text-foreground">Payment</div>
        ...
        <div className="px-4 pt-4">
          <div className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider mb-2">Order Summary</div>
          <div className="bg-card rounded-3xl border border-border overflow-hidden">
            {[
              ...
              <div key={i} className="flex justify-between items-center px-4 py-3 border-b border-border last:border-b-0">
            ...
            <div className="flex justify-between items-center px-4 py-3 border-t border-border">
              ...
              <span className="text-xl font-extrabold text-primary">{formatRWF(total)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 pt-5">
          <div className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider mb-2">Payment Method</div>
          <div className="space-y-2">
            {payMethods.map(pm => (
              <button
                ...
                className={`w-full flex items-center gap-3 bg-card border rounded-2xl px-4 py-4 transition-all ${
                  pm.disabled ? 'opacity-40 cursor-not-allowed border-border' :
                  selected === pm.id ? 'border-primary' : 'border-border'
                }`}
              >
                <div className={`w-[46px] h-[46px] rounded-xl ${pm.bgClass} flex items-center justify-center text-2xl`}>
                  {pm.emoji}
                </div>
                <div className="flex-1 text-left">
                  <div className="text-sm font-bold text-foreground">{pm.name}</div>
                  <div className={`text-xs mt-0.5 ${pm.badge ? 'text-primary font-bold' : 'text-muted-foreground font-medium'}`}>{pm.sub}</div>
                </div>
      ...
      <div className="sticky bottom-0 left-0 right-0 bg-card border-t border-border p-4 z-40">
        <button
          onClick={handlePay}
          disabled={loading}
          className="w-full bg-primary text-primary-foreground rounded-full py-4 text-base font-extrabold flex items-center justify-center gap-2 active:scale-[0.98] transition-all disabled:opacity-50"
        >
